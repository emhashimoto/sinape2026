...
$$\lambda(t_i|X_i)=\lambda_0(t_i)Z_j\exp(X_{ij}^{\top}\beta), \quad i=1,\ldots,n_j, \quad j=1\ldots k$$
Alternativamente, usando $Z_i = \exp(W_i)$, em que $W_i$ é o efeito aleatório aditivo:

$$\lambda(t | X_{ij}, W_i) = \lambda_0(t) \exp(X_{ij}^{\top} \beta + W_i)$$
...
$$L_{marg}(\beta, \theta) = \prod_{i=1}^k \int_{0}^{\infty} \left[ \prod_{j=1}^{n_i} L_{ij}(t_{ij} | Z_i) \right] f_{\Gamma}(z_i; 1/\theta, 1/\theta) dz_i$$
...
$$L_{marg}(\beta, \sigma^2) = \prod_{i=1}^k \int_{-\infty}^{\infty} \left[ \prod_{j=1}^{n_i} L_{ij}(t_{ij} | W_i) \right] \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{w_i^2}{2\sigma^2}\right) dw_i$$
...
<table class="tabela-cox">
  <thead>
    <tr class="group-row">
      <th rowspan="2" style="text-align:left; vertical-align:bottom; padding-bottom:14px;">Variável</th>
